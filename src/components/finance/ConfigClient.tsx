...
function ItemsTab({ initialItems }: { initialItems: ItemData[] }) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const body = {
        ...
        maxPerMonth: form.maxPerMonth ? parseInt(form.maxPerMonth) : null,
      };
      const url = editingItem
        ? `/api/finance/config/items/${editingItem.id}`
        : "/api/finance/config/items";
  ...
  const handleToggleActive = useCallback(async (item: ItemData) => {
    try {
      const res = await fetch(`/api/finance/config/items/${item.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = useCallback(async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/finance/config/items/${id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const data = await res.json();
            ...
            >
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {!item.active && (
                    <Badge className="bg-zinc-500/15 text-zinc-400 text-[10px]">
                      Inactivo
                    </Badge>
                  )}
                </div>
                <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-0.5 text-xs text-muted-foreground">
                  ...
                  {item.maxPerDay != null && (
                    <span>Máx/día: {fmtCLP.format(item.maxPerDay)}</span>
                  )}
                  {item.maxPerMonth != null && (
                    <span>Máx/mes: {fmtCLP.format(item.maxPerMonth)}</span>
                  )}
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, maxPerDay: e.target.value })
                  }
                  className="mt-1"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, maxPerMonth: e.target.value })
                  }
                  className="mt-1"
                />
              </div>
...
function KmParamsTab({
  initialConfig,
}: {
  initialConfig: ConfigData | null;
}) {
  const [kmPerLiter, setKmPerLiter] = useState(
    initialConfig?.kmPerLiter?.toString() ?? "10"
  );
  const [fuelPrice, setFuelPrice] = useState(
    initialConfig?.fuelPricePerLiter?.toString() ?? "1500"
  );
  const [feePct, setFeePct] = useState(
    initialConfig?.vehicleFeePct?.toString() ?? "10"
  );
  const [maxDaily, setMaxDaily] = useState(
    initialConfig?.maxDailyAmount?.toString() ?? ""
  );
  const [maxMonthly, setMaxMonthly] = useState(
    initialConfig?.maxMonthlyAmount?.toString() ?? ""
  );
  const [pendingDays, setPendingDays] = useState(
    initialConfig?.pendingAlertDays?.toString() ?? "5"
  );
  const [approvalDays, setApprovalDays] = useState(
    initialConfig?.approvalAlertDays?.toString() ?? "3"
  );
  const [santanderAccount, setSantanderAccount] = useState(
    initialConfig?.santanderAccountNumber ?? ""
  );
  const [saving, setSaving] = useState(false);
  ...
  const handleSave = useCallback(async () => {
    setSaving(true);
    try {
      const res = await fetch("/api/finance/config/params", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          kmPerLiter: parseFloat(kmPerLiter) || 10,
          fuelPricePerLiter: parseInt(fuelPrice) || 1500,
          vehicleFeePct: parseFloat(feePct) || 10,
          maxDailyAmount: maxDaily ? parseInt(maxDaily) : null,
          maxMonthlyAmount: maxMonthly ? parseInt(maxMonthly) : null,
  ...
  return (
    <div className="space-y-4 max-w-xl">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Configura cómo se calcula el costo de los trayectos.
        </p>
      </div>
      ...
      <Card>
        <CardContent className="pt-5 space-y-4">
          ...
          <div className="border-t border-border pt-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label htmlFor="pendingDays">
                  Alerta rendición pendiente (días)
                </Label>
                <Input
                  id="pendingDays"
                  type="number"
              ...
              <div>
                <Label htmlFor="approvalDays">
                  Alerta aprobación pendiente (días)
                </Label>
                <Input
                  id="approvalDays"
                  type="number"
          ...
          <div className="border-t border-border pt-4">
            <Label htmlFor="santanderAccount">
              Cuenta origen Santander
            </Label>
            <Input
              id="santanderAccount"
              value={santanderAccount}
              onChange={(e) => setSantanderAccount(e.target.value)}
...
function ApproversTab({
  ...
  const [approver1, setApprover1] = useState(
    initialConfig?.defaultApprover1Id ?? ""
  );
  const [approver2, setApprover2] = useState(
    initialConfig?.defaultApprover2Id ?? ""
  );
  const [saving, setSaving] = useState(false);
  ...
  const handleSave = useCallback(async () => {
    setSaving(true);
    try {
      const res = await fetch("/api/finance/config/approvers", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          defaultApprover1Id: approver1 || null,
          defaultApprover2Id: approver2 || null,
        }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Error");
  ...
  return (
    <div className="space-y-4 max-w-xl">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Define quién aprueba las rendiciones. Puedes configurar hasta 2 niveles.
        </p>
      </div>
        ...
        <CardContent className="pt-5 space-y-4">
          <div>
            ...
            <Select value={approver1} onValueChange={setApprover1}>
              ...
              <SelectContent>
                <SelectItem value="">Sin aprobador</SelectItem>
...
function RulesTab({
  ...
  const [requireImage, setRequireImage] = useState(
    initialConfig?.requireImage ?? true
  );
  const [requireObservations, setRequireObservations] = useState(
    initialConfig?.requireObservations ?? false
  );
  const [requireTollImage, setRequireTollImage] = useState(
    initialConfig?.requireTollImage ?? false
  );
  const [saving, setSaving] = useState(false);
  ...
  const handleSave = useCallback(async () => {
    setSaving(true);
    try {
      const res = await fetch("/api/finance/config/rules", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          requireImage,
  ...
  const rules = [
    {
      id: "requireImage",
      label: "Requerir imagen de comprobante",
      description:
        "Los usuarios deben adjuntar al menos una imagen al crear una rendición.",
      enabled: requireImage,
      toggle: () => setRequireImage(!requireImage),
    },
    {
      id: "requireObservations",
      label: "Requerir observaciones",
      description:
        "El campo de descripción/observaciones es obligatorio.",
      enabled: requireObservations,
      toggle: () => setRequireObservations(!requireObservations),
    },
    {
      id: "requireTollImage",
      label: "Requerir imagen de peaje",
      description:
        "Para rendiciones de kilometraje con peaje, se requiere imagen del comprobante.",
      enabled: requireTollImage,
      toggle: () => setRequireTollImage(!requireTollImage),
    },
  ];
  ...
  return (
    <div className="space-y-4 max-w-xl">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Configura qué campos son obligatorios al crear rendiciones.
        </p>
      </div>
...
function CostCentersTab({
  ...
  initialCostCenters: CostCenterData[];
}) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const url = editingCC
        ? `/api/finance/config/cost-centers/${editingCC.id}`
        : "/api/finance/config/cost-centers";
  ...
  const handleToggleActive = useCallback(async (cc: CostCenterData) => {
    try {
      const res = await fetch(
        `/api/finance/config/cost-centers/${cc.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = useCallback(async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/finance/config/cost-centers/${id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const data = await res.json();
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Define centros de costo para clasificar rendiciones.
          </p>
        </div>
        <Button size="sm" onClick={openCreate}>
          <Plus className="h-4 w-4 mr-1" />
        ...
        <div className="space-y-1.5">
          {costCenters.map((cc) => (
            <div
              ...
              className={cn(
                "flex items-center justify-between gap-3 p-3 rounded-lg border transition-colors",
                cc.active ? "border-border" : "border-border/50 opacity-60"
              )}
            >
