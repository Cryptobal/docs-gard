 ...
 * GET - Verificar documentos por vencer y vencidos, crear notificaciones
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // We need to check each document's alertDaysBefore individually
      ...
      if (expDate <= alertDate && expDate > today) {
        ...
        const existing = await prisma.notification.findFirst({
          where: {
            ...
            createdAt: { gte: addDays(today, -1) }, // Don't send duplicate within 24h
          },
        });
          ...
          await prisma.$transaction([
            ...
            prisma.notification.create({
              data: {
                ...
                message: `Vence el ${format(expDate, "dd/MM/yyyy")}. Quedan ${Math.ceil(
